...
* Если на сервере уже был установлен другой WireGuard-совместимый VPN на той же подсети (например прошлая установка AmneziaWG 1.5 до этого форка) — `setup.sh`/`client.sh` сами добавляют более точные маршруты на каждого пира, чтобы обратный трафик не терялся из-за конфликта с чужим маршрутом на ту же `/24` (без этого хендшейк проходит, а данные — молча нет).
...
Google агрессивно размечает целые пулы адресов Cloudflare WARP как Россию — не по конкретному IP, а по всей подсети конкретного дата-центра (colo), через который вы туда попадаете. Anycast-маршрут к Cloudflare зависит от сети вашего хостера/ASN, а не от страны сервера, поэтому два сервера в разных странах с разными ASN вполне могут попадать в разные colo — но **какой конкретно colo окажется "чистым" по разметке Google, заранее не угадать и не зафиксировать как правило**: пул адресов одного и того же colo может быть RU-размечен целиком сегодня и частично очищен завтра (или наоборот). Проверить свой colo: `curl -s https://1.1.1.1/cdn-cgi/trace | grep colo` — но сам номер colo ничего не говорит о его текущей "чистоте" без отдельной проверки геолокации (YouTube/Gemini) через этот же тоннель.
Вместо того чтобы полагаться на чей-то устаревший список "чистых" colo, в панели есть раздел **WARP Geo**: он гоняет реальные гео-проверки (Cloudflare trace, YouTube, Gemini) через уже поднятый тоннель узла, а кнопка предпросмотра Cloudflare позволяет проверить актуальную гео-разметку через одноразовый Cloudflare-тоннель **не переключая провайдера и не трогая рабочий Proton-тоннель**. Если весь пул вашего текущего colo скомпрометирован, ротация эндпоинтов (пункт 8 раздела «Настройка») не поможет в принципе — переключайтесь на Proton VPN (пункт 7 там же), у него нет этой проблемы вообще.
