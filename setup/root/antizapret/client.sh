...
render() {
	...
	while read -r line; do
		...
		# AmneziaWG 2.0 не допускает пустые I1-I5 (пустой junk-слот "I2 =" без
		# значения - невалидная строка конфига, спека требует просто не включать
		# неиспользуемые слоты). Наши шаблоны/пресеты маскировки оставляют I2-I5
		# пустыми чаще, чем заполненными (TLS/QUIC маски используют только I1) -
		# строгие клиенты (сам `awg setconf`, не только официальные приложения)
		# отказываются парсить такой файл целиком.
		[[ "$line" =~ ^I[1-5]\ =\ *$ ]] && continue
		echo "$line"
	done < "$1"
}
